...
NTL.PillData.items = {
	...
	-- /// dyes ///
	redpaint = { types = { "dye" }, skillrequirement = 0, effects = { { type = "color", r = 255, g = 51, b = 51 } } },
	greenpaint = { types = { "dye" }, skillrequirement = 0, effects = { { type = "color", r = 51, g = 255, b = 51 } } },
	bluepaint = { types = { "dye" }, skillrequirement = 0, effects = { { type = "color", r = 51, g = 51, b = 255 } } },
	blackpaint = { types = { "dye" }, skillrequirement = 0, effects = { { type = "color", r = 51, g = 51, b = 51 } } },
	whitepaint = {
		types = { "dye" },
		skillrequirement = 0,
...
NTL.PillData.combos = {
	...
	sodiumboom1 = {
		...
		effectoverride = { skillrequirement = 0, effects = { { type = "addtag", tag = "instantexplode" } } },
	},
	sodiumboom2 = {
...
function NTL.PillConfigToTags(config)
	...
	if config.description ~= nil then
		table.insert(res, "des/" .. config.description)
	end
	...
	if config.color ~= nil then
		table.insert(
			...
			"col/"
				...
				.. "/"
				.. tostring(HF.Round(config.color[3]))
		)
	end
...
function NTL.PillConfigFromItems(components, skill, descriptionOverride, user)
	...
	local res =
		{ fx = {}, capacity = 0, yield = 2, tags = {}, ingredients = {}, color = nil, description = nil, sprite = nil }
	...
	for i, itemidentifier in ipairs(components) do
		...
		if NTL.PillData.items[itemidentifier] ~= nil then
			s = math.random() < skill / (NTL.PillData.items[itemidentifier].skillrequirement or 30)
		end
		success[i] = s
	end
	...
	for i, itemidentifier in ipairs(components) do
		if NTL.PillData.items[itemidentifier] ~= nil then
			...
			if not success[i] then
				effects = NTL.PillData.items[itemidentifier].faileffects or effects
			end

			for effect in effects do
				if effect.type == "potencymult" then
					potencymult = potencymult * effect.value
				elseif effect.type == "basepotencymult" then
					basepotencymult = basepotencymult * effect.value
				elseif effect.type == "binderpotencymult" then
					binderpotencymult = binderpotencymult * effect.value
				elseif effect.type == "activepotencymult" then
					activepotencymult = activepotencymult * effect.value
				elseif effect.type == "fillerpotencymult" then
					fillerpotencymult = fillerpotencymult * effect.value
				elseif effect.type == "yieldmult" then
					yieldmult = yieldmult * effect.value
				elseif effect.type == "yield" then
					yield = yield + effect.value
				elseif effect.type == "capacity" then
					capacity = capacity + effect.value
				elseif effect.type == "sprite" then
					res.sprite = effect.value
				end
			end
		end
	end
	...
	for comboidentifier, combo in pairs(NTL.PillData.combos) do
		...
		if combo.requireditems ~= nil then
			for requirement in combo.requireditems do
				if
					res.ingredients[requirement.id] == nil
					or res.ingredients[requirement.id] < (requirement.amount or 1)
				then
					viable = false
					break
				end
	...
	if activecombo ~= nil and activecombo.effectoverride ~= nil then
		local s = math.random() < skill / (activecombo.effectoverride.skillrequirement or 30)
		local effects = nil
		if s or activecombo.effectoverride.faileffects == nil then
			effects = activecombo.effectoverride.effects
		else
			effects = activecombo.effectoverride.faileffects
		end
		...
		if effects ~= nil then
			for effect in effects do
				if effect.type == "addeffect" and (effect.chance == nil or HF.Chance(effect.chance)) then
					...
					if res.fx[effect.identifier] ~= nil then
						strength = strength + res.fx[effect.identifier]
					end
					-- set effect
				...
				for effect in effects do
					if effect.type == "addeffect" and (effect.chance == nil or HF.Chance(effect.chance)) then
						...
						if itemType == "base" then
							strengthmult = basepotencymult
						elseif itemType == "binder" then
							strengthmult = binderpotencymult
						elseif itemType == "active" then
							strengthmult = activepotencymult
						elseif itemType == "filler" then
							strengthmult = fillerpotencymult
						end
	...
	if activecombo ~= nil then
		...
		if activecombo.descriptionoverride ~= nil then
			descriptionOverride = TextManager.Get("pillcombo." .. activecombo.descriptionoverride).Value
...
function NTL.RefreshPillDescription(item)
	...
	if config.sprite ~= nil then
		identifier = "custompill_" .. config.sprite
	end
