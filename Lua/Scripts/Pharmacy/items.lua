...
function NTL.TryCraftPills(chemmaster, user, dontreporterrors)
	if chemmaster == nil or user == nil then
		return false
	end
	if dontreporterrors == nil then
	...
	ingredients.active = {}
	for i = 7, 12, 1 do
		table.insert(ingredients.active, inv.GetItemAt(i))
	end
	...
	for categorykey, category in pairs(ingredients) do
		for item in category do
			...
			if itemdata == nil then
				errors[#errors + 1] =
					HF.ReplaceString(TextManager.Get("lua.chemerror.invaliditem").Value, "{id}", item.Name)
			elseif itemdata.types[1] ~= categorykey then
				errors[#errors + 1] = HF.ReplaceString(
					TextManager.Get("lua.chemerror.invalidcategory." .. categorykey).Value,
					"{id}",
					item.Name
				)
			else
	...
	if #ingredients.base <= 0 then
		errors[#errors + 1] = TextManager.Get("lua.chemerror.missingbase").Value
	end
	if #ingredients.binder <= 0 then
		errors[#errors + 1] = TextManager.Get("lua.chemerror.missingbinder").Value
	end
	...
	for categorykey, category in pairs(ingredients) do
		for item in category do
			ingredientArray[#ingredientArray + 1] = item.Prefab.Identifier.Value
		end
	end
	...
	local config =
		NTL.PillConfigFromItems(ingredientArray, HF.GetSkillLevel(user, "medical"), descriptionOverride, user)
	local productidentifier = "custompill"
	if config.sprite ~= nil then
		productidentifier = "custompill_" .. config.sprite
	end
	...
	if #ingredients.base > 0 and #ingredients.active > config.capacity then
		errors[#errors + 1] =
			HF.ReplaceString(TextManager.Get("lua.chemerror.capacity").Value, "{cap}", tostring(config.capacity))
	end
	...
	if not dontreporterrors then
		HF.GiveItem(user, "ntpsfx_chemaccept")
	end
	return true
end
...
Hook.Add("NTL.Chemalyzer.rename", "NTL.Chemalyzer.rename", function(effect, deltaTime, item, targets, worldPosition)
	...
	if containedItem.Condition == 1 then
		return
	end
