...
Timer.Wait(function()
	...
	NT.Afflictions.bloodpressure = {
		...
		update = function(c, i)
			...
			c.afflictions.bloodpressure.strength = HF.Clamp(
				HF.Round(HF.Lerp(c.afflictions.bloodpressure.strength, desiredbloodpressure, bloodpressurelerp)),
				5,
				200
			)
		end,
	...
	local function InfuseBloodPlasma(item, packtype, usingCharacter, targetCharacter, limb)
		...
		local compatible = (packhasantibodyA and packhasantibodyB)
			or (targethasantibodyO and not packhasantibodyC)
			or targethasantibodyC
			or (targethasantibodyA and packhasantibodyA)
			or (targethasantibodyB and packhasantibodyB)
			...
			if limbaff[i].strength < 50 then
				limbaff[i].strength = limbaff[i].strength
					- (c.afflictions.immunity.prev / 3000 + HF.Clamp(limbaff.bandaged.strength, 0, 1) * 0.1)
						* c.stats.healingrate
						* NT.Deltatime
						* (HF.Clamp(c.afflictions.afbloodplasma.strength, 0, 1) + 1)
			end
		end,
	}

	-- Organ damage
	NT.Afflictions.cerebralhypoxia = {
		max = 200,
		update = function(c, i)
			if c.stats.stasis then
				return
			...
			local gain = (
				...
				+ (c.afflictions.liverdamage.strength / 800)
					* (1 - HF.Clamp(c.afflictions.afbloodplasma.strength, 0, 1)) -- from liverdamage
