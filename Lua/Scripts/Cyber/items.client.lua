...
NTL.AllowedNecromancyItemsStartsWith = {
	crowbar = 1,
	screwdriver = 1,
	organscalpel = 1,
}
...
local function itemAllowsNecromancy(item)
	...
	if NTL.AllowedNecromancyItems[identifier] ~= nil then
		return true
	else
		for key, _ in pairs(NTL.AllowedNecromancyItemsStartsWith) do
...
Hook.Patch("Barotrauma.CharacterHealth", "OnItemDropped", function(instance, ptable)
	if not instance.Character.IsDead then
		return
	end -- no necromancy required
	if ptable.ReturnValue == false then
		return
	end -- dropped outside of the health UI

	if not itemAllowsNecromancy(ptable["item"]) then
		return
	end
