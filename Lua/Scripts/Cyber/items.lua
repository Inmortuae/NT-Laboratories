...
NTL.ItemMethods.cyberarm = function(item, usingCharacter, targetCharacter, limb)
	...
	if NTL.HF.LimbIsCyber(targetCharacter, limbtype) then
		return
	end
	if
		not (
			NT.LimbIsSurgicallyAmputated(targetCharacter, limbtype)
			or HF.HasAfflictionLimb(targetCharacter, "bonecut", limbtype, 99)
		)
	then
		return
	end
	if limbtype ~= LimbType.LeftArm and limbtype ~= LimbType.RightArm then
		return
	end
...
NTL.ItemMethods.waterproofcyberleg = function(item, usingCharacter, targetCharacter, limb)
	...
	if limbtype ~= LimbType.LeftLeg and limbtype ~= LimbType.RightLeg then
		return
	end
...
NTL.ItemStartsWithMethods.crowbar = function(item, usingCharacter, targetCharacter, limb)
	...
	local isGoodCondition = not HF.HasAfflictionLimb(targetCharacter, "ntc_materialloss", limbtype, 20)
	...
	if
		isGoodCondition
		and (
			HF.GetSkillRequirementMet(usingCharacter, "mechanical", 50)
			or HF.GetSkillRequirementMet(usingCharacter, "medical", 70)
		)
	then
...
NTL.ItemStartsWithMethods.screwdriver = function(item, usingCharacter, targetCharacter, limb)
	...
	if HF.GetAfflictionStrengthLimb(targetCharacter, limbtype, "ntc_loosescrews", 0) < 0.1 then
		return
	end
...
local function possiblyRejectOrgan(targetCharacter, usingCharacter, organName)
	...
	if
		HF.Chance(rejectionchance)
		and NTConfig.Get("NT_organRejection", false)
		and not HF.HasAfflictionLimb(targetCharacter, "ntc_cyberkidney", LimbType.Torso, 0.1)
	then
...
local function implantOrgan(item, usingCharacter, targetCharacter, limb)
	...
	if not HF.GetSkillRequirementMet(usingCharacter, NTL.OrganConfigDatas[organName].secondarySkillName, 60) then
...
local function implantBrain(item, usingCharacter, targetCharacter, limb)
	...
	if
		not HF.HasAffliction(targetCharacter, organName .. "removed", 1)
...
NT.ItemMethods.handcuffkey = function(item, usingCharacter, targetCharacter, limb)
	if not targetCharacter.IsDead then
		return
	end -- if they're alive, the XML works fine
...
Timer.Wait(function()
		...
		if targetCharacter.IsDead then
			...
			print(
				"scalpel limbtype: "
					.. tostring(limbtype)
					.. "vs head "
					.. tostring(LimbType.Head)
					.. " and "
					.. tostring(targetCharacter.AnimController.GetLimb(limbtype))
			)
			if targetCharacter.AnimController.GetLimb(limbtype) == nil then
				return
			end -- can't scalpel a missing head
	...
	NT.ItemMethods.advretractors = function(item, usingCharacter, targetCharacter, limb)
		if targetCharacter.IsDead then
			...
			if
				(
					HF.HasAfflictionLimb(targetCharacter, "surgeryincision", limbtype, 99)
					or HF.HasAfflictionLimb(targetCharacter, "clampedbleeders", limbtype, 99)
	...
	NT.ItemMethods.organscalpel_kidneys = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTL.OrganConfigDatas["kidney"])
	end
	NT.ItemMethods.organscalpel_liver = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTL.OrganConfigDatas["liver"])
	end
	NT.ItemMethods.organscalpel_lungs = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTL.OrganConfigDatas["lung"])
	end
	NT.ItemMethods.organscalpel_heart = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTL.OrganConfigDatas["heart"])
	end
	NT.ItemMethods.organscalpel_brain = function(p1, p2, p3, p4)
		removeCyberOrgan(p1, p2, p3, p4, NTL.OrganConfigDatas["brain"])
	end
