...
NTC.AddOnDamagedHook(function (characterHealth, attackResult, hitLimb)
    ...
    if causeDamageTypeConversion and NTL.HF.LimbIsCyber(targetChar,hitLimb.type) then
        if sfxidentifier ~= nil then HF.GiveItem(targetChar,sfxidentifier) end
        Timer.Wait(function() NTL.ConvertDamageTypes(targetChar,hitLimb.type) end,1)
    end
end)
...
NT.DislocateLimb = function(character,limbtype,strength)
    ...
    if strength > 0 and NTL.HF.LimbIsCyber(character,limbtype) then return end
