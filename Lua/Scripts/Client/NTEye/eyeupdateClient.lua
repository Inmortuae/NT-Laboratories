--timer values
NTL.ClientUpdateCooldown = 0
NTL.ClientUpdateInterval = 120
...
Hook.Add("think", "NTL.updatetriggerclient", function()

    if HF.GameIsPaused() or (not Level.Loaded) then return end
	
    NTL.ClientUpdateCooldown = NTL.ClientUpdateCooldown-4
	
    if (NTL.ClientUpdateCooldown <= 0) then
        NTL.ClientUpdateCooldown = NTL.ClientUpdateInterval
        NTL.UpdateHumanEyeEffect()
    end
...
function NTL.UpdateHumanEyeEffect()
	...
	elseif HF.HasAffliction(Character.Controlled, "eyeterror") then
			...
			NTL.RobotraumaClientPatch(LevelLight)
			
		end
end

--sends a request for HUD items to spawn in case lua fucks up
function NTL.SendItemSpawnRequest() 

	if not Game.IsMultiplayer then NTL.SpawnEffectItems() return end --singleplayer comp
...
function NTL.writeHUDs()
	...
	if HF.HasAffliction(Character.Controlled, "medicallens") then
		...
		if eyeHUD==nil then
			...
			then 
				NTL.ResetHUDvalues()
				NTL.GetClientItemValues() 
				NTL.SendItemSpawnRequest() 
			return end
...
function NTL.disableHUDs()

	if NTL.checkHUDs() then 
...
function NTL.GetClientItemValues()
...
function NTL.RobotraumaClientPatch() end 