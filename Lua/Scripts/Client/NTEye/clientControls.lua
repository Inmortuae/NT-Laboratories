...
function NTL.PlayBeepSound(soundTarget)
...
function NTL.HUDActivation()
	...
	then
		...
		then
			NTL.disableHUDs()
			DeactivatedHUDs=1
			NTL.PlayBeepSound(Character.Controlled)
		elseif
...
function NTL.Zoom(instance,ptable)
		...
		then
			...
			then
				...
				then 
					...
					NTL.ScannerActive = 1
					NTL.PlayScannerSound(scannerTarget)
					Timer.Wait(function() NTL.ScannerActive = 0 end, 500)
				return end
				...
				NTL.HealthScanner(scannerUser, scannerTarget, limb)
