...
Hook.Add("spoonUsed", "NTL.eyestealing", function(effect, dt, item, targets, targetCharacter, worldpos)
		...
		then 
			...
			then
				if --has eyes
					...
					and	not NTL.HasEyeProtection(spoontarget)
				then
					NTL.SpoonEyeDetect(spoontarget)
					
					NTL.ClearCharacterEyeAfflictions(spoontarget)
...
function NTL.SpoonEyeDetect(character)
...
function NTL.PlayScream(character)
