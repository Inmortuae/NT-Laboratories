NTL.UpdateCooldown = 0
NTL.UpdateInterval = 120
NTL.Deltatime = NTL.UpdateInterval/60 -- Time in seconds that transpires between updates

-- This Hook triggers NTL.Update function.
Hook.Add("think", "NTL.updatetriggerserver", function()
    if HF.GameIsPaused() or (not Level.Loaded) then return end

    NTL.UpdateCooldown = NTL.UpdateCooldown-1
    if (NTL.UpdateCooldown <= 0) then
        NTL.UpdateCooldown = NTL.UpdateInterval
        NTL.Update() 
    end
end)

--checks if character is in diving gear on demand  (old code breaks when I touch it)
function NTL.IsInDivingGear(character)
...
function NTL.HasEyeProtection(character)
	local outerSlot = character.Inventory.GetItemInLimbSlot(InvSlotType.OuterClothes)
	local headSlot = character.Inventory.GetItemInLimbSlot(InvSlotType.Head)
...
function NTL.GetItemInSlot(character, slot)
...
function NTL.UpdateHumanEye(character)

	if  	--checks if there are eyes
		NTL.HasEyes(character)
	then
	...
	--print(character.IsImmuneToPressure)
		...
		if 		--glasses check for eyezoomClient.lua
					NTL.GetItemInSlot(character, InvSlotType.Head) 
				and NTL.GetItemInSlot(character, InvSlotType.Head).Prefab.identifier == "eyeglasses" 
		...
		then
					NTL.ClearCharacterEyeAfflictions(character) -- in eyesurgeryServer.lua
		...
		then
					HF.AddAfflictionLimb(character, "deusizinedrop", 11, -1.5)
		end
...
-- Gets to run once every two seconds triggers NTL.UpdateHumanEye
function NTL.Update()
	--print("eyeupdatetest")
