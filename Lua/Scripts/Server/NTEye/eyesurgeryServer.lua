...
function NTL.HasEyes(targetCharacter)
...
function NTL.ClearCharacterEyeAfflictions(character)
  eyeaffs = {
...
function NTL.CataractClearAfflictions(character)
	cateyeaffs = {
		"corneaincision",
		"emulsification",
		"eyecataract"
...
function NTL.GiveItemBasedOnEye(character, usingCharacter)

	if NTL.HasEyes(character) then
		...
		if HF.HasAffliction(character, "eyebionic") then --bionic
			...
			NTL.LensRemoval(character, usingCharacter)
...
--Surgery Code
Hook.Add("item.applyTreatment", "NTL.Surgery", function(item, usingCharacter, targetCharacter, limb)
	...
	then
		...
		then
			...
			then
				...
				then
					NTL.GiveItemBasedOnEye(targetCharacter, usingCharacter)
					
					NTL.ClearCharacterEyeAfflictions(targetCharacter)
				...
				then
					...
					item.OwnInventory.GetItemAt(0).Condition = item.OwnInventory.GetItemAt(0).Condition-100
						...
						then 
							...
							NTL.PlayScream(targetCharacter)
...
--------------------------------------------BIONIC LENS SURGERY-----------------------------------------------------
	...
	then
		...
		then
			...
			then
				NTL.LensRemoval(targetCharacter, usingCharacter)
			else
...
function NTL.LensRemoval(targetCharacter, usingCharacter)
