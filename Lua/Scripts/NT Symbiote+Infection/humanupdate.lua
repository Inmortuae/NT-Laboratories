...
Timer.Wait(function()
    ...
    NT.Afflictions.surgery_huskhealth = {max=100,update=function(c,i)
        ...
        if not NTL.HF.HasHusk(c.character) then c.afflictions[i].strength=0 return end
    ...
    NT.Afflictions.bonecuttorso = {max=100,update=function(c,i)
        ...
        if c.afflictions[i].strength >= 99 and c.afflictions.surgery_huskhealth.strength <= 0.1 and NTL.HF.HasHusk(c.character) then
    ...
    NT.Afflictions.huskinfection = {max=100,update=function(c,i)
        if c.stats.stasis then return end
        if c.afflictions[i].strength<80 then return end
    ...
    NT.Afflictions.huskinfectionstasis = {max=60,update=function(c,i)
        if c.stats.stasis then return end
        if c.afflictions[i].strength<1 then return end
        ...
        c.afflictions.heartdamage.strength= c.afflictions.heartdamage.strength-0.1 * NT.Deltatime end
        if c.afflictions.lungdamage.strength<50 then
        c.afflictions.lungdamage.strength= c.afflictions.lungdamage.strength-0.1 * NT.Deltatime end
        if c.afflictions.liverdamage.strength<50 then
        c.afflictions.liverdamage.strength= c.afflictions.liverdamage.strength-0.1 * NT.Deltatime end
        ...
        elseif c.afflictions.kidneydamage.strength<99 then
            c.afflictions.kidneydamage.strength= HF.Clamp(c.afflictions.kidneydamage.strength-0.1 * NT.Deltatime,51,100)
        end

        -- do some good
        if c.afflictions.analgesia.strength<40 then
            c.afflictions.analgesia.strength= c.afflictions.analgesia.strength-0.5 * NT.Deltatime end
