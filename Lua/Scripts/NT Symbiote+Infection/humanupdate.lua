...
Timer.Wait(function()
		...
		update = function(c, i)
			...
			if c.afflictions[i].strength > 0 then
				...
				if (prevHuskInfection > 25 or prevHuskInfection < 10) and prevHuskInfection < 99 then
					HF.SetAffliction(c.character, "huskinfection", newHuskInfection)
				end
			end
		end,
	}

	NT.Afflictions.husklamp = {
		max = 10,
		update = function(c, i)
			if c.afflictions[i].strength > 0.1 then
				...
				if c.afflictions.lungdamage.strength < 99 then
					c.afflictions.lungdamage.strength = c.afflictions.lungdamage.strength - 0.04 * NT.Deltatime
				end

				-- limb specifics
				for limbType in limbTypes do
					if HF.GetAfflictionStrengthLimb(c.character, limbType, "ointmented") < 10 then
						HF.AddAfflictionLimb(c.character, "ointmented", limbType, 2 * NT.Deltatime)
					end
					if HF.GetAfflictionStrengthLimb(c.character, limbType, "burn") < 50 then
						HF.AddAfflictionLimb(c.character, "burn", limbType, -0.2 * NT.Deltatime)
					end
				end
			end
		end,
	...
	NT.Afflictions.surgery_huskhealth = {
		max = 100,
		update = function(c, i)
			if c.stats.stasis then
			...
			if not NTL.HF.HasHusk(c.character) then
				c.afflictions[i].strength = 0
				return
			end
			if not HF.HasAffliction(c.character, "bonecuttorso", 99) then
				c.afflictions[i].strength = 0
				return
			end
	...
	NT.Afflictions.bonecuttorso = {
		...
		update = function(c, i)
			...
			if
				...
				and c.afflictions.surgery_huskhealth.strength <= 0.1
				and NTL.HF.HasHusk(c.character)
			then
				HF.GiveItem(c.character, "ntsfx_huskhurt")
				c.afflictions.surgery_huskhealth.strength =
					HF.Clamp(100 - c.afflictions.af_calyxanide.strength, 80, 100)
			end
		end,
	}

	-- making Husk Infection treat neurotrauma stuff
	NT.Afflictions.huskinfection = {
		max = 100,
		update = function(c, i)
			if c.stats.stasis then
				return
			...
			c.afflictions.internalbleeding.strength = c.afflictions.internalbleeding.strength - 0.05 * NT.Deltatime
			...
			if c.afflictions.lungdamage.strength < 25 then
			...
			for limbType in limbTypes do
				if HF.GetAfflictionStrengthLimb(c.character, limbType, "gangrene") < 15 then
					HF.AddAfflictionLimb(c.character, "gangrene", limbType, -0.1 * NT.Deltatime)
				end
				-- arterial bleeds
				NT.ArteryCutLimb(c.character, limbType, -0.05 * NT.Deltatime)
				-- foreign bodies
				HF.AddAfflictionLimb(c.character, "foreignbody", limbType, -0.05 * NT.Deltatime)
				-- bleeding
				HF.AddAfflictionLimb(c.character, "bleeding", limbType, -0.05 * NT.Deltatime)
			end

			-- wait a tick with this because otherwise it'll be removed before being used
			Timer.Wait(function()
				NTC.SetMultiplier(c.character, "hypoxemiagain", 0.1)
			end, 1)

			-- Infection stasis
			local shouldBeInStasis = c.afflictions.cardiacarrest.strength > 0.1
				or c.afflictions.cerebralhypoxia.strength > 102
				or c.afflictions.coma.strength > 20
				or c.character.Vitality < 0
				or c.character.bloodloss > 90

			if shouldBeInStasis then
				c.afflictions.huskinfectionstasis.strength = 60
			end
		end,
	}

	NT.Afflictions.husksymbiosis = {
		max = 100,
		update = function(c, i)
			if c.stats.stasis then
				return
			...
			c.afflictions.hemotransfusionshock.strength = 0
			c.afflictions.pneumothorax.strength = 0
			c.afflictions.bonedamage.strength = c.afflictions.bonedamage.strength - 0.25 * NT.Deltatime
			...
			c.afflictions.lungdamage.strength = c.afflictions.lungdamage.strength - 0.5 * NT.Deltatime
			c.afflictions.oxygenlow.strength = c.afflictions.oxygenlow.strength - 100 * NT.Deltatime
			NTC.SetSymptomFalse(c.character, "hyperventilation", 2)
			NTC.SetSymptomFalse(c.character, "hypoventilation", 2)
			c.afflictions.respiratoryarrest.strength = 0
			...
			for limbType in limbTypes do
				...
				-- arterial bleeds
				NT.ArteryCutLimb(c.character, limbType, -0.25 * NT.Deltatime)
				-- foreign bodies
				HF.AddAfflictionLimb(c.character, "foreignbody", limbType, -0.5 * NT.Deltatime)
				-- bleeding
				HF.AddAfflictionLimb(c.character, "bleeding", limbType, -0.5 * NT.Deltatime)
			end
	...
	NT.Afflictions.huskinfectionstasis = {
		max = 60,
		update = function(c, i)
			if c.stats.stasis then
				return
			...
			-- treat stuff
			c.afflictions.internalbleeding.strength = c.afflictions.internalbleeding.strength - 0.5 * NT.Deltatime
			c.afflictions.coma.strength = c.afflictions.coma.strength - 1 * NT.Deltatime
			c.afflictions.tamponade.strength = c.afflictions.tamponade.strength - 0.5 * NT.Deltatime
			c.afflictions.bonedamage.strength = c.afflictions.bonedamage.strength - 0.5 * NT.Deltatime
			c.afflictions.traumaticshock.strength = c.afflictions.traumaticshock.strength - 0.5 * NT.Deltatime
			HF.SetAffliction(c.character, "cpr_buff_auto", 2)

			-- heal organs
			if c.afflictions.heartdamage.strength < 50 then
				c.afflictions.heartdamage.strength = c.afflictions.heartdamage.strength - 0.1 * NT.Deltatime
			end
			if c.afflictions.lungdamage.strength < 50 then
				c.afflictions.lungdamage.strength = c.afflictions.lungdamage.strength - 0.1 * NT.Deltatime
			end
			if c.afflictions.liverdamage.strength < 50 then
				c.afflictions.liverdamage.strength = c.afflictions.liverdamage.strength - 0.1 * NT.Deltatime
			end
			if c.afflictions.kidneydamage.strength < 50 then
				c.afflictions.kidneydamage.strength = c.afflictions.kidneydamage.strength - 0.1 * NT.Deltatime
			elseif c.afflictions.kidneydamage.strength < 99 then
				c.afflictions.kidneydamage.strength =
					HF.Clamp(c.afflictions.kidneydamage.strength - 0.1 * NT.Deltatime, 51, 100)
			end

			-- do some good
			if c.afflictions.analgesia.strength < 40 then
				c.afflictions.analgesia.strength = c.afflictions.analgesia.strength - 0.5 * NT.Deltatime
			end
			if c.afflictions.seizure.strength < 20 then
			...
			for limbType in limbTypes do
				-- arterial bleeds
				NT.ArteryCutLimb(c.character, limbType, -1 * NT.Deltatime)
				-- fractures
				if HF.Chance(0.005) then
					NT.BreakLimb(c.character, limbType, -1000)
				end
				-- foreign bodies
				HF.AddAfflictionLimb(c.character, "foreignbody", limbType, -0.1 * NT.Deltatime)
			end
	...
	NT.Afflictions.huskstasis = {
		...
		update = function(c, i)
			...
			if c.afflictions.heartdamage.strength < 99 then
				c.afflictions.heartdamage.strength = c.afflictions.heartdamage.strength - 0.5 * NT.Deltatime
			end
			if c.afflictions.lungdamage.strength < 99 then
				c.afflictions.lungdamage.strength = c.afflictions.lungdamage.strength - 0.5 * NT.Deltatime
			end
